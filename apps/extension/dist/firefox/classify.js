 ...
 */
function classifyFieldLocal(fieldHint, value) {
    const v = (value || "").trim();
    if (!v)
        return null; // empty: nothing to report (caller sends removal)
    const hint = (fieldHint || "").toLowerCase();
    // 1. Explicit secret-looking VALUES win regardless of field.
    if (/sk-[A-Za-z0-9\-_]{4,}/.test(v))
        return "SECRET";
    if (/^(api[_-]?key|secret|token|bearer)\s*[:=]/i.test(v))
        return "SECRET";
    ...
    if (hintSecret && v.length >= 3)
        return "SECRET";
    if (hintEmail)
        return "EMAIL";
    if (hintPhone)
        return "PHONE";
    if (hintName && /^[A-Za-z][A-Za-z .'\-]{1,40}$/.test(v))
        return "NAME";
    // 3. Value-pattern fallback for unlabelled fields.
    if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v))
        return "EMAIL";
    if (/^\+?[\d][\d\s\-().]{6,}$/.test(v) && (v.replace(/\D/g, "").length >= 7))
        return "PHONE";
    if (v.length >= 16 && /^[A-Za-z0-9\-_+/=]{16,}$/.test(v) && /[A-Za-z]/.test(v) && /\d/.test(v))
        return "SECRET";
    if (/^[A-Z][a-z]+(?: [A-Z][a-z]+){0,3}$/.test(v))
        return "NAME";
    // 4. Ordinary text.
    return "TEXT";
}
